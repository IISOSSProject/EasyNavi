...
{
    class LocalSourceDataProvider : ISourceDataProvider
    {
        static string LocalSourceDataPath { get; } = "EasyNavi.Core.Resources.LocalSourceData.";
        static string LocalSourceDataFile { get; } = $"{LocalSourceDataPath}ContentsData.zip";
        ...
        {
            var text = await ZipFileHelper.ReadStringFromResource(_assembly, LocalSourceDataFile, $"detail_{contentId}.json");
        ...
        {
            using (var stream = _assembly.GetManifestResourceStream($"{LocalSourceDataPath}Versions.json"))
